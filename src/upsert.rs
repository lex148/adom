...
pub fn impl_upsert_macro(ast: &syn::DeriveInput) -> TokenStream {
    let name = &ast.ident;
    ...
    let upsert_cols: Vec<String> = my_fields
        .iter()
        .filter(|&f| f.ident.as_ref().unwrap().to_string() != "id")
        .filter(|f| utils::is_upsert_col(f))
        .map(|f| utils::get_columnname_forced(f))
        .filter_map(|f| f)
        .collect();

    assert!(
        upsert_cols.len() > 0,
        "ERROR: can not compile an AdomUpsert without any AdomUpsertColumn"
    );

    let insert_str = insert_text(&tablename, my_fields, &upsert_cols);

    let gen_create_no_audit = quote! {
        impl #name {
            pub async fn upsert<C>(&mut self, client: &C) -> std::result::Result<(), tokio_postgres::error::Error>
    ...
    let gen_create_with_audit = quote! {
        impl #name {
            pub async fn upsert<C>(&mut self, client: &C, by: &#created_by_type) -> std::result::Result<(), tokio_postgres::error::Error>
                where C: GenericClient
            {
...
fn insert_text(table: &str, fields: &syn::Fields, upsert_cols: &[String]) -> String {
    ...
    let cols = cols.join(", ");
    let args = args.join(", ");
    let upsert_cols_text = upsert_cols.join(", ");
    format!(
        "INSERT INTO {table} ({cols}) VALUES ({args}) 
        ON CONFLICT ({upsert_cols})
        DO UPDATE SET id = {table}.id 
        RETURNING id",
        table = table,
        cols = cols,
        args = args,
        upsert_cols = upsert_cols_text
    )
}
